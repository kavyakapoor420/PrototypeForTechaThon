...
//     setFullName(event.target.value); // Update fullName state as the user types
//   };

//   const redirectToExperience = (event) => {
//     event.preventDefault();

//     // Store the username (full name) in localStorage
//     if (PanId) {
//       localStorage.setItem('PanId', PanId);
//     }

//     // Redirect to experience.html
...
//             onChange={handleNameChange} // Update state as user types
...
const SignupPage = () => {
  const [typingText, setTypingText] = useState("");
  const fullText = "Create Your Account";
  const [PanId, setPanId] = useState(""); // State for PanId
  ...
  const handlePanIdChange = (event) => {
    setPanId(event.target.value); // Update PanId state as the user types
  };

  const redirectToExperience = (event) => {
    event.preventDefault();

    // Store the PanId in localStorage
    if (PanId) {
      localStorage.setItem('userPAN', PanId);
    }
    ...
    <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-b from-green-900 to-gray-900 text-white relative overflow-hidden">
      ...
      <div className="signup-box bg-gray-900 p-8 rounded-2xl shadow-2xl w-96 relative transition mt-12 transform hover:scale-105">
        <form onSubmit={redirectToExperience}>
          ...
          {/* PanId input field */}
          <label className="block text-gray-400 text-sm mb-1">Enter your PanId</label>
          <input
            type="text"
            placeholder="Enter your PanId"
            required
            value={PanId} // Bind the input to the PanId state
            onChange={handlePanIdChange} // Update PanId as user types
            className="w-full p-3 mb-3 bg-gray-700 text-white rounded-full border border-gray-600 focus:outline-none focus:ring-2 focus:ring-green-500 transition"
          />
